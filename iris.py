...
df.loc[df['target'] == 1, 'target'] = 'virsicolor'
...
# インプットデータ（1行のデータフレーム）
value_df = pd.DataFrame([], columns=['data', 'sepal length (cm)', 'petal length'])
...
pred_df = pd.DataFrame(pred_probs, columns=['setosa', 'versicolor', 'virsinica'], index=['probability'])
st.write('## Prediction')
st.write(pred_df)
...
st.write('## Result')
st.write(f'このアイリスはきっと{name[0]}です！')